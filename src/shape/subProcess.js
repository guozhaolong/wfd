...
export default function (G6) {
    G6.registerNode('sub-process-node', {
        ...
        drawShape(cfg, group) {
            ...
            group.anchorShapes = [];
            group.showAnchor = (_group) => {
                this.drawAnchor(_group);
            };
            group.clearAnchor = (_group) => {
                if (_group.anchorShapes) {
                    _group.anchorShapes.forEach(a => a.remove());
                }
                _group.anchorShapes = [];
            };
            group.clearHotpotActived = (_group) => {
                if (_group) {
                    _group.anchorShapes.forEach(a => {
                        if (a.isAnchor)
